...
def again(): #<----- FUNCTION HEADER
    '''this function ask a user if they'd like to enter another temp, checks the respose for validity, and then returns a valid reponse back to the main program'''
    
    ans = input("\t\tWould you like to enter another temperature? [y/n]: ").lower()

    #while loop trap - ensure user provdies valid value before moving on
...
def converter(f):
    '''this function is passed a tempF value ('f' paramater), converts to celsius, and returns the tempC value'''
...
while answer == "y":
    ...
    temp_count = temp_count + 1 #tempcount += 1
    temp_total += tempF
...
avg_tempF = temp_total / temp_count #temp_c
